...
import { BarbellIcon, ConfettiIcon, SwordsIcon, SoccerBallIcon, TrophyIcon } from './icons';

interface EventConfiguratorProps {
  /**
   * 'roster' offers the event types built from the selected players;
   * 'listless' the ones that have no participant list at all.
   */
  mode: 'roster' | 'listless';
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted mb-4">
        {mode === 'listless'
          ? 'Eventos sin lista de participantes. Elegí el tipo:'
      ...
      <div className="flex flex-col gap-3">
        {mode === 'listless' ? (
          ...
          >
            <ConfettiIcon className="w-5 h-5" />
            <span className="font-normal">
              Evento, solo fecha, horario y lugar
            </span>
          </button>
        ) : (
          <>
